...
function wc_bb_payments_gateway_load()
{
    ...
    class WC_Gateway_BB_Payments extends WC_Payment_Gateway
    {
        ...
        function get_payment_args($order)
        {
            ...
            $args = array(
                'UserKey' => $UserKey,
                'GoodURL' => str_replace('https:', 'http:', add_query_arg('wc-api', 'WC_Gateway_BB_Payments', home_url('/'))) . "&user_key=$UserKey",
                'ErrorURL' => str_replace('https:', 'http:', add_query_arg('wc-api', 'WC_Gateway_BB_Payments', home_url('/'))) . "&user_key=$UserKey",
                //'ErrorURL' => str_replace('amp;', '', $order->get_cancel_order_url()),
                'CancelURL' => str_replace('amp;', '', $order->get_cancel_order_url()),
...
//            }
        ...
        function check_ipn_response()
        {
	        ...
	        $received_values = stripslashes_deep($_GET);
                ...
                wp_die("<h1 style='color: red;'>Ошибка!</h1><p>Оплата не прошла. Проверьте правильность заполнения данных кредитной карты.</p><p>Принимаются только международные карты с достаточным балансом для оплаты заказа.</p><p>Для повторной попытки оплаты вернитесь на <a href='$cancelURL'>страницу оплаты</a>.</p><br/><br/><p>С уважением, <br/>администрация Международной академии каббалы</p>" . print_r($_GET, true));
            }
        }
